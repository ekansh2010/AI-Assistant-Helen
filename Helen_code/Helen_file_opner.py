...
async def index_files(base_dirs):
    ...
    logger.info(f"✅ from the {base_dirs} in total {len(file_index)} files are indexed.")
    return file_index

async def search_file(query, index):
    choices = [item["name"] for item in index]
    if not choices:
        logger.warning("⚠ No files available to match.")
        return None
...
async def open_file(item):
    try:
        logger.info(f"📂 File is opening : {item['path']}")
        if os.name == 'nt':
        ...
        return f"✅ File is opened: {item['name']}"
